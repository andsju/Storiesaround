...
if(isset($_REQUEST['token'])){
	...
	if ($_REQUEST['token'] == $_SESSION['token']) {
		...
		if(isset($_REQUEST['pages_folder'])){
			...
			$images_filename = isset($_REQUEST['images_filename']) ? $_REQUEST['images_filename'] : "";
			...
			if(array_key_exists('success', $result)) {
				...
				foreach ($sizes as $size) {
					$f = substr($images_filename, 0, $pos) .'_'.$size . $ext;
					if (is_file($p . $f)) {
						unlink($p . $f);
					}
				}
				...
				//database update
				$save = new Pages();
				$save->replacePagesImages($pages_images_id, $filename, $ratio, $sizes_to_db, $image_description, $artist, $xmpdata);
			}
