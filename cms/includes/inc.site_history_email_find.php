...
if(isset($_POST['btn_search'])) {
	$email_to = filter_input(INPUT_POST, 'email_to', FILTER_SANITIZE_STRING) ? $_POST['email_to'] : null;
} else {
	$email_to = filter_input(INPUT_POST, 'email_to', FILTER_SANITIZE_STRING) ? $_POST['email_to'] : null;
}
...
?>
	...
	if (isset($_REQUEST['history_email_find'])) {
		if (strlen($_REQUEST['history_email_find']) > 0) {
			$arr_words = explode(" ", filter_var($_REQUEST['history_email_find'], FILTER_SANITIZE_STRING));
		}
	}
