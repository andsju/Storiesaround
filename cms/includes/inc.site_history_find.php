...
<?php
	...
	if(isset($_POST['btn_search'])) {
		...
		$module = isset($_POST['module']) ? filter_var(trim($_POST['module']), FILTER_SANITIZE_STRING) : null;
		$action = isset($_POST['action']) ? filter_var(trim($_POST['action']), FILTER_SANITIZE_STRING) : null;
	} else {
		...
		$module = isset($_GET['module']) ? filter_var(trim($_GET['module']), FILTER_SANITIZE_STRING) : null;
		$action = isset($_GET['action']) ? filter_var(trim($_GET['action']), FILTER_SANITIZE_STRING) : null;
	}
	...
	if (isset($_REQUEST['history_find'])) {
		if (strlen($_REQUEST['history_find']) > 0) {
			$arr_words = explode(" ", filter_var($_REQUEST['history_find'], FILTER_SANITIZE_STRING));
		}
	}
