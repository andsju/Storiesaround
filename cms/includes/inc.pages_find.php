...
if (isset($_POST['btn_pages_search'])) {
    if (strlen($_POST['pages_find']) > 0) {
        $search = filter_var($_POST['pages_find'], FILTER_SANITIZE_STRING);
    }
    $form = true;
...
if (!$large_table) {
    ...
    if ($row_pages) {
        ...
        foreach ($row_pages as $r) {
            ...
            $html .= '<td>' . $r['title'] . '</td>';
            $tag = strlen($r['tag']) ? '<span class="ui-icon ui-icon-tag" style="display:inline-block;" title="' . $r['tag'] . '"></span>' : null;
            $html .= '<td>' . $tag . '</td>';
