...
class Selections extends Database
{
     ...
     * @return array
     */
    public function getMultipleSelectionsContent($pages_selections_ids)
    {
        $in = trim(str_repeat('?,', count($pages_selections_ids)), ',');
		...
		FROM pages_selections 
		WHERE pages_selections_id IN ($in)
		AND active = 1
		ORDER BY position ASC";

        $stmt = $this->db->prepare($sql);
        // $stmt->bindParam(':id', $pages_selections_id, PDO::PARAM_STR);
        $stmt->execute($pages_selections_ids);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
