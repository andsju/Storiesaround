...
if (isset($_GET['token'])){
	...
	if ($_GET['token'] == $_SESSION['token']) {
		...
		$page_title = "Upload and replace image";
		...
		<script>
			$(document).ready(function() {		
				...
				var uploader = new qq.FileUploader({
					...
					action: 'pages_images_upload_replace_ajax.php',
					...
					template: 
						...
						'<div class="qq-upload-button" style="width:100%;height:50px;font-size:1.2em;padding:20px">Upload and replace image (click or drop)</div>' +
						'<ul class="qq-upload-list"></ul>' + 
						'</div>',
					
					//debug: true,
					params: {token: token, pages_folder: pages_folder, max_width: max_width, original: original, replace: replace, images_filename: images_filename, pages_images_id: pages_images_id},
					sizeLimit: 10520000,
        ...
        <input type="hidden" name="images_filename" id="images_filename" value="<?php echo $_GET['images_filename'];?>">
        <input type="hidden" name="pages_images_id" id="pages_images_id" value="<?php echo $_GET['pages_images_id'];?>">
