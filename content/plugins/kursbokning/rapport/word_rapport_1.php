...
$i = 2;
foreach($rows as $row) {
	$personnummer = strlen($row['personnummer']) > 0 ? '('. $row['personnummer']. ')' : '';
	$section->writeText($row['fnamn'] . ' '. $row['enamn']. ' '.$personnummer, $font_body_text, $paragraph_body_text_start);
	$section->setNoBreak();
	$section->writeText($row['adress'], $font_body_text, $paragraph_body_text);
	$section->setNoBreak();
	$section->writeText($row['postnummer']. ' '. $row['ort']. ' '. $row['country'], $font_body_text, $paragraph_body_text);
	$section->setNoBreak();
	$section->writeText('Tel: '.$row['telefon']. ' | Mobil: '. $row['mobil'], $font_body_text, $paragraph_body_text);
	$section->setNoBreak();
	$section->writeText('E-post: '.$row['epost'], $font_body_text, $paragraph_body_text);
	$section->setNoBreak();
	if(strlen($row['kommun']) > 0) {
		$section->writeText($row['kommun']. ', '. $row['lan'], $font_body_text, $paragraph_body_text);
		$section->setNoBreak();
	}
	if(strlen($row['organisation']) > 0) {
		$section->writeText('Organisation: '.$row['organisation'], $font_body_text, $paragraph_body_text);
		$section->setNoBreak();
	}
	$section->setNoBreak();
	if(strlen($row['fakturaadress']) > 0) {
		$section->writeText('Fakturaadress:', $font_body_text, $paragraph_body_text);
		$section->setNoBreak();
		$section->writeText($row['fakturaadress'], $font_body_text, $paragraph_body_text);
	}
	$section->setNoBreak();
	$utc_created = ($row['utc_created']>'2000-01-01 00:00') ? new DateTime(utc_dtz($row['utc_created'], $dtz, 'Y-m-d H:i:s')) : null;
	$section->writeText('Datum: '.$utc_created->format('Y-m-d'), $font_body_text, $paragraph_body_text_end);
	//$section->setNoBreak();	
	$ids = explode(',',$row['plugin_kursbokning_kurser_id']);
	$prio = 1;
	foreach($ids as $id) {
		$r = $kursbokning->getKursbokningKursId($id);
		$section->writeText($prio .': '. $r['title'], $font_style_small, $paragraph_body_text);
		$prio++;
	}
